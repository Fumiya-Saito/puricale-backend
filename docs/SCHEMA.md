...
| `reminder_days` | `integer`| `3` | | 行事の事前通知タイミング（日数）。1=1日前, 3=3日前 など |
| `daily_api_usage` | `integer`| `0` | | 【AI悪用対策】本日のAPI（AI）利用回数カウンター |
| `last_api_used_date`| `date` | null | | 最後にAPIを利用した日付（日本時間） |
